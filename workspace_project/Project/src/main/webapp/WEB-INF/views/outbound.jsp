            ...
            <div style="position: relative; overflow: hidden; width: 100%; height: 100%;">
                <div class="!translate-x-[5.5%] pr-4 xl:!-mr-8 xl:w-[calc(100%_+_20px)]"
                    ...
                    <div class="flex h-full flex-col justify-between">
                        <div>
                                <span class="absolute top-4 block cursor-pointer text-zinc-200 xl:hidden "><svg
                                        ...
                                        <a href="<%=request.getContextPath()%>/outbound" style="border-radius: 0px; width: 100%;">
                                            <div class="w-full items-center justify-center">
                                                <div class="flex w-full items-center justify-center">
                                                    ...
                                                    <p class="mr-auto text-sm font-semibold text-white">출고 현황</p>
                                                </div>
                                            </div>
                                        </a>
                                    </div>
                        ...
                        <div class="mt-7 mb-9">
                            ...
                            <div class="mt-5 flex w-full items-center rounded-lg border border-zinc-200 p-4 ">
                                <a href="/shadcn-nextjs-boilerplate/dashboard/dashboard/settings"><span
                                        class="relative flex h-10 min-h-10 w-10 min-w-10 shrink-0 overflow-hidden rounded-full"><img
    ...
    <nav
        ...
        <div class="ml-[6px]">
            <p class="text-md shrink text-zinc-950 capitalize md:text-3xl"><a
                    class="font-bold capitalize hover:text-zinc-950" href="#" style="border-radius: 0px;">출고
                현황</a></p>
        </div>
            ...
            <div class="overflow-hidden rounded bg-white shadow">
                <table class="min-w-full text-gray-700">
                    ...
                    <tr>
                        ...
                        <th class="p-4 text-center">주문수량</th>
                        <th class="p-4 text-center">주문일시</th>
                        <th class="p-4 text-center">출고완료일시</th>
                        <th class="p-4 text-center">구매기업명</th>
                        <th class="p-4 text-center">상태</th>
                    </tr>
                    </thead>
                    <tbody>
                    <c:forEach var="order" items="${outboundList}">
                        <tr class="border-b border-gray-300 hover:bg-gray-50">
                            <td class="p-4"><input type="checkbox" /></td>
                            <td class="p-4 text-center">${order.orderId}</td>
                            <td class="p-4 text-center">${order.productName}</td>
                            <td class="p-4 text-center">${order.orderQtty}</td>
                            <td class="p-4 text-center">
                                <fmt:formatDate value="${order.orderDate}" pattern="yyyy-MM-dd a hh:mm" />
                            </td>
                            <td class="p-4 text-center">
                                <fmt:formatDate value="${order.outboundDate}" pattern="yyyy-MM-dd a hh:mm" />
                            </td>
                            <td class="p-4 text-center">${order.companyName}</td>

                            <td class="relative text-center group h-[40px]">
                                <c:choose>
                                    <c:when test="${order.status == 20}">
                                        <!-- 상태 뱃지 -->
                                        <span class="inline-block rounded-md bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-700 relative z-10">출고 준비중</span>
                                        ...
                                        <form method="post"
                                            ...
                                            <button type="submit"
                                                    ...
                                                    class="rounded-md bg-blue-500 px-2 py-1 text-xs font-medium text-white hover:bg-blue-600">
                                                출고 진행
                                            </button>
                                        </form>
                                    </c:when>
                                    ...
                                    <c:when test="${order.status == 21}">
                                        <!-- 상태 뱃지 -->
                                        <span class="inline-block rounded-md bg-blue-100 px-2 py-1 text-xs font-medium text-blue-700 relative z-10">
                                            출고 진행중</span>

                                        <!-- 출고 완료 버튼 (hover 시 등장) -->
                                        <form method="post"
                                              action="${pageContext.request.contextPath}/outbound/complete"
                                              class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
                                            <input type="hidden" name="orderId" value="${order.orderId}" />
                                            <button type="submit"
                                                    onclick="return confirm('출고 완료 처리하시겠습니까?');"
                                                    class="rounded-md bg-green-500 px-2 py-1 text-xs font-medium text-white hover:bg-green-600">
                                                출고 완료
                                            </button>
                                        </form>
                                    </c:when>

                                    <c:when test="${order.status == 22}">
                                        <span class="inline-block rounded-md bg-green-100 px-2 py-1 text-xs font-medium text-green-700">출고 완료</span>
                                    </c:when>

                                    <c:otherwise>
                                        <span class="inline-block rounded-md bg-gray-200 px-2 py-1 text-xs font-medium text-gray-600">상태 확인 필요</span>
                                    </c:otherwise>
                                </c:choose>
                            </td>
